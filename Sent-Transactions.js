...
const tableValues=document.getElementsByClassName("Values");
...
const mainWallet=web3.eth.accounts.wallet;//To create horizontal bar, Account information should be gotten from this.
      ...
      //Map from network to Etherscan:
      const networkToEtherscan=new Map();
            networkToEtherscan.set("Ethereum Mainnet","https://etherscan.io/tx/");
            networkToEtherscan.set("Ropsten (Test Network)","https://ropsten.etherscan.io/tx/");
            networkToEtherscan.set("Kovan (Test Network)","https://kovan.etherscan.io/tx/");
            networkToEtherscan.set("Rinkeby (Test Network)","https://rinkeby.etherscan.io/tx/");
            networkToEtherscan.set("Goerli (Test Network)","https://goerli.etherscan.io/tx/");
...
logOut.addEventListener("click", walletLogout);
      searchButton.addEventListener("click", function(){ txSearch((searchBox.value).toString())});
      moreDetails.addEventListener("click", function(){window.location.href=networkToEtherscan.get(providerToNetwork.get(web3.eth.currentProvider.url))+tableValues[3].innerHTML});
        ...
        try{
         ...
         function gotoDeleteAccount(){
                ...
                function gotoSendTransaction(){
                    ...
                    function txSearch(pointer){
                            pointer=pointer.trim();
                        txTable.style.visibility="visible";
                        try{
                                if(pointer.includes(":")){
                                const seperator=pointer.indexOf(":");
                                const blockHash=pointer.slice(0,seperator);
                                const txIndex=pointer.slice(seperator+1);
                                        try{
                                                if((web3.utils.isHexStrict(blockHash))||(Number.isInteger(Number(blockHash))&&blockHash>=0)){
                                                        web3.eth.getTransactionFromBlock(blockHash,txIndex).then(function(txObject){
                                                                web3.eth.getBlock(txObject.blockHash).then(function(block){
                                                                        const blockTimestamp=new Date(block.timestamp*1000);
                                                                        timestampValue.innerHTML=blockTimestamp.toUTCString();
                                                                        tableValues[0].innerHTML=txObject.blockHash;
                                                                        tableValues[1].innerHTML=txObject.blockNumber;
                                                                        tableValues[2].innerHTML=parseInt(txObject.chainId,16);
                                                                        tableValues[3].innerHTML=txObject.hash;
                                                                        tableValues[4].innerHTML=txObject.transactionIndex;
                                                                        tableValues[5].innerHTML=txObject.from;
                                                                        tableValues[6].innerHTML=txObject.to;
                                                                        tableValues[7].innerHTML=web3.utils.fromWei(txObject.value)+" ETH";
                                                                        tableValues[8].innerHTML=txObject.gas;
                                                                        tableValues[9].innerHTML=web3.utils.fromWei(txObject.gasPrice)+" ETH";
                                                                        tableValues[10].innerHTML=web3.utils.fromWei(txObject.gas*txObject.gasPrice+txObject.value)+" ETH";
                                                                        tableValues[11].innerHTML=block.miner;
                                                                });
                                                });
                                                }else{
                                        ...
                                        try{
                                                if(web3.utils.isHexStrict(pointer)){
                                                        web3.eth.getTransaction(pointer).then(function(txObject){
                                                        web3.eth.getBlock(txObject.blockHash).then(function(block){
                                                                const blockTimestamp=new Date(block.timestamp*1000);
                                                                timestampValue.innerHTML=blockTimestamp.toUTCString();
                                                                tableValues[0].innerHTML=txObject.blockHash;
