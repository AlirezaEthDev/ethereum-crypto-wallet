...
const jsonFilePassword=document.getElementById("jsonFilePassword");
...
//Setting events:
      ...
      privateKey.addEventListener("change", privateKeyChanging);
      jsonFile.addEventListener("change", jsonFileChanging);
      add.addEventListener("click", importAccount);
            function privateKeyChanging(){
                if(privateKey.value==""){
            ...
            function jsonFileChanging(){
                if(this.files.length==0){
                        privateKey.disabled=false;
                        jsonFilePassword.value="";
                        jsonFilePassword.disabled=true;
                }else{
                        privateKey.disabled=true;
                        jsonFilePassword.disabled=false;
                }
            }
            function importAccount(){
                try{
                    if(jsonFile.files.length==0&&privateKey.value==""){
                            throw "You should add an account with one of the two ways!";
                    }else if(jsonFile.files.length==1&&jsonFilePassword.value==""){
                            throw "You should provide a password for the JSON file!"
                    }else{
                        if(privateKey.disabled){
                                if(window.confirm("This account will be added to "+providerToNetwork.get(web3.eth.currentProvider.url)+"!")){
                                        const fileReader= new FileReader();
                                        fileReader.readAsText(jsonFile.files[0]);
                                        fileReader.onload=function(){
                                                const decryptedAccount=web3.eth.accounts.decrypt(fileReader.result, jsonFilePassword.value);
                                                        mainWallet.add(decryptedAccount);
                                                        mainWallet.save(statusPad);
                                                        window.location.href="View-New-Account.html";
                                                        //Next page shows the new account's address, private key, index, balance and nonce;
                                        }
                                }
                        }else{
                                if(window.confirm("This account will be added to "+providerToNetwork.get(web3.eth.currentProvider.url)+"!")){
                                        mainWallet.add(privateKey.value);
                                        mainWallet.save(statusPad);
                                        window.location.href="View-New-Account.html";
                                }
                        }
