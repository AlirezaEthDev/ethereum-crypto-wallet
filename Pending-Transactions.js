...
let pendingTXCounter=0;
let pendingTXListener;
...
pendingTXList.addEventListener("click", gotoPendingTransactionDetails);
stopButton.addEventListener("click",pendingListStop);
              window.alert("To view pending transactions you should choose a network from the horizontal bar.\nTo view pending transactions on another network reload the page and choose the network!")
        ...
        try{
         ...
         function gotoDeleteAccount(){
                ...
                function gotoSendTransaction(){
                    ...
                    function pendingTXListLoader(txHash){
                        let iTagDetails=document.createElement("i");
                            iTagDetails.className="fas fa-eye";
                            iTagDetails.style.fontSize="larger";
                            pendingTXList.style.visibility="visible";
                            pendingTXCounter++;
                            listTitle.innerHTML="Pending Transactions ("+pendingTXCounter+")";
                            let newRow=pendingTXList.insertRow();
                                newRow.insertCell().appendChild(iTagDetails);
                                newRow.insertCell().textContent=pendingTXCounter;
                                newRow.insertCell().textContent=txHash;
            }
            function gotoPendingTransactionDetails(event){
                if(event.target.className=="fas fa-eye"){
                    const selectedHash=event.target.parentNode.nextSibling.textContent;
                          //localStorage.setItem("transactionHash", event.target.parentNode.nextSibling.textContent);
                          //window.alert(localStorage.getItem("transactionHash"));
                          mainWallet.save(statusPad);
                          window.location.href="https://ropsten.etherscan.io/tx/"+selectedHash;
                }
            }
            ...
            function pendingListStop(){
                    if(stopButton.innerHTML=="Stop"){
                        pendingTXListener.unsubscribe(function(err,result){
                                if(err){
                                        window.alert(err);
                                }else{
                        ...
                        pendingTXListener=web3.eth.subscribe("pendingTransactions", function(err,result){
                                if(err){
                                    window.alert(err);
                                }else{
                                        stopButton.innerHTML="Stop";
                                }
                            }).on("data", function(pendingList){pendingTXListLoader(pendingList)});
                    }
